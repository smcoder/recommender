...
import org.smcoder.recommend.business.model.domain.Product;
import org.smcoder.recommend.business.model.domain.User;
import org.smcoder.recommend.business.model.domain.Recommendation;
import org.smcoder.recommend.business.model.dto.*;
import org.smcoder.recommend.business.utils.Constant;
import org.apache.log4j.Logger;
import org.smcoder.recommend.business.handler.*;
...
public class ProductController {
    ...
    @Autowired
    private RecommenderHandler recommenderService;
    @Autowired
    private ProductHandler productHandler;
    @Autowired
    private UserHandler userService;
    @Autowired
    private RatingHandler ratingService;
    ...
    public Model getHotProducts(@RequestParam("num") int num, Model model) {
        ...
        try {
            ...
            model.addAttribute("products", productHandler.getRecommendProducts(recommendations));
        } catch (Exception e) {
    ...
    public Model getProductInfo(@PathVariable("id") int id, Model model) {
        model.addAttribute("success", true);
        model.addAttribute("product", productHandler.findByProductId(id));
        return model;
    }
    ...
    public Model getSearchProducts(@RequestParam("query") String query, Model model) {
        ...
        try {
            products = productHandler.findByProductName(query);
            model.addAttribute("success", true);
